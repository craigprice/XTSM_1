        ...
        def dispatch(self,destinations_=None):
            ...
            for dest in self.destinations:
                ...
                try: 
                    ...
                    print packed_message.__class__
                    #flag = self.server.send(packed_message,dest, isBinary=True)
                    #packed_message = msgpack.packb({"IDLSocket_ResponseFunction":'databomb','data_context':'default:127.0.0.1','databomb':'data'})
                    #flag = self.server.send(packed_message,dest, isBinary=False)
                    flag = self.server.send(simplejson.dumps({'Not_Command_text_message':'hi','terminator':'die'}, ensure_ascii = False).encode('utf8'),dest)
                    #flag = True
                    if flag:
                        print "dest:"
                        print dest
