...
class MulticastProtocol(DatagramProtocol):
    ...
    def datagramReceived(self, datagram_, address):
        ...
        ping_command = ServerCommand(self.server, self.server.catch_ping, datagram)
...
class WSClientProtocol(WebSocketClientProtocol):
    ...
    def onMessage(self, payload, isBinary):
        ...
        msg_command = ServerCommand(self.server, self.connection.on_message, payload, isBinary, self)
...
class ConnectionManager(XTSM_Server_Objects.XTSM_Server_Object):
    ...
    def __init__(self, server):
        ...
        maintenance_command = ServerCommand(self.server, self.periodic_maintainence)
...
class ScriptServer(GlabClient):
    ...
    def on_open(self):
        ...
        if len(self.connection_manager.script_servers) < MAX_SCRIPT_SERVERS:
            script_command = ServerCommand(self.server, self.connection_manager.add_script_server)
...
class DataGuiServer(GlabClient):
    ...
    def open_connection(self):
        ...
        if DEBUG: print "About to open"
        data_gui_path = os.path.abspath(data_guis.__file__)
...
#Loopiong Call, under server.chck for new image on cameradatabomb -> databomb dispatcher - 
    ...
    def compile_active_xtsm(self, params):
        ...
        '''
        want to change to the following:CP 10-20-2014
        dc = self.__determineContext__(params)
        if not dc.dict.has_key('_exp_sync'):
            exp_sync = Experiment_Sync_Group(self.server)
            dc.update({'_exp_sync':exp_sync})
        dc['_exp_sync'].shotnumber = int(params['shotnumber'])
        if DEBUG: print "datacontext name:", dc.name
        
        if params['data_context'] == 'PXI_emulator':
            dc = self.__determineContext__(params)
            if not dc.dict.has_key('_exp_sync'):
                exp_sync = Experiment_Sync_Group(self.server, dc.name)#pass the reference of dc
                dc.update({'_exp_sync':exp_sync})
            dc['_exp_sync'].shotnumber = int(params['shotnumber'])
        dc = self.__determineContext__(params)
        '''
        if params['data_context'] == 'PXI': #'169.254.174.200:8083':
        ...
        if not dc.dict.has_key('_exp_sync'): 
            self.server.broadcast('{"server_console": "'+
                                ...
                                ' requested timing data in data_context: ' + dc.name + ', but nothing is ' +
                                'assigned to run on this system.", "data_context":"' + dc.name + '"}')
            params['request']['protocol'].transport.loseConnection()
            return
        ...
        str(datetime.now()) +  " Parsing started" + " Shotnumber = " + str(sn) + '", "data_context":"' + dc.name + '"}')
        
        if not hasattr(xtsm_object, 'XTSM'):
            if DEBUG: print "Error: Bad XTSM:"
        ...
        self.server.broadcast('{"server_console": "' +
                                 str(datetime.now()) +
                                 " Parsing finished" +
                                 " Shotnumber = " + str(sn) +  '", "data_context":"' + dc.name + '"}')
        self.server.broadcast('{"parsed_active_xtsm": "' +
                                 str(datetime.now()) + '", "data_context":"' + dc.name + '"}')
        ...
        for script in xtsm_object.XTSM.getDescendentsByType('Script'):
        ...
        self.server.dataContexts['PXI_emulator'].update({'Test_instrument':glab_instrument.Glab_Instrument(params={'server':self.server,'create_example_pollcallback':True})})
        ...
        def _changed_xtsm(changedelm):
            self.server.broadcast('{"xtsm_change":"'+str(sn)+'", "data_context":"' + dc.name + '"}')
    ...
    def testparse_active_xtsm(self, params):
        ...
        if DEBUG: print datetime.now(), " Parsing started", " Shotnumber= ",sn
        XTSMobjectify.preparse(xtsm_object)
        parserOutput = xtsm_object.parse(sn)
        XTSMobjectify.postparse(parserOutput)
        if DEBUG: print datetime.now(), " Parsing finished", " Shotnumber= ",sn
    ...
    def databomb(self, params):
        ...
        print "asdf"
        #pdb.set_trace()
        packed_message = msgpack.packb({"IDLSocket_ResponseFunction":'set_global_variable_from_socket','data_context':dc.name,'packed_databomb':params['databomb']}, use_bin_type=True)#self.packed_data
    ...
    def temp_plot_qt(self,params,bomb_id, dc):
        ...
        raw_databomb = msgpack.unpackb(params['packed_databomb'])
    ...
    def temp_plot(self, params, bomb_id, dc):
        ...
        if num_pics != 0:
            if num_pics == 1:
                ...
                if DEBUG: print "Not Supported"
                raw_img_data[1] = np.asarray(raw_img_data[1],dtype=int)
                return
                raise
        #min_scale = 65536
        min_scale = np.amin(raw_img_data[1])
        ...
        #region_of_interest = corrected_image[1:5, #down, specify bottom,
        #                                     1:5]#second number is how far accross
        #pdb.set_trace()
        ...
        plt.savefig(path+'/'+file_name+'.png')
        if DEBUG: print "Shotnumber:", str(raw_databomb['shotnumber'])
...
class GlabPythonManager():
    ...
    def _init_when_running(self):
        ...
        self.command_queue.add(ServerCommand(self.server, self.connection_manager.add_script_server))
        self.command_queue.add(ServerCommand(self.server, self.connection_manager.add_data_gui_server))
    ...
    def server_ping(self):
        ...
        server_command = ServerCommand(self.server, self.server_ping)
