...
#        tsh[26:32]= Reserved for future use
    ...
    def construct_denseT(self):
        ...
        self.denseT=numpy.array([self.channelMap.hardwaretime])       
        ...
        try: 
            ...
            # check the time seperation between two neighbors.  Added by JZ on 2/12/2014      
            sepcheck=self.denseT[1:]-self.denseT[0:-1]
            # if the seperation is smaller than the timingresolution, coerce the time to the tg's timingresolution.    Added by JZ on 2/12/2014       
            for si in range(len(sepcheck)):
                if sepcheck[si]<self.clockgenresolution:
                    self.denseT[si]=((self.denseT[si]/self.clockgenresolution).round())*self.clockgenresolution
                    self.denseT[si+1]=((self.denseT[si+1]/self.clockgenresolution).round())*self.clockgenresolution
            self.denseT=numpy.unique(self.denseT)
        except: pass
...
class Interval(gnosis.xml.objectify._XO_,XTSM_core):
    ...
    def parse_harvest(self, dense_time_array):
        ...
        # The update time for this interval is extracted using index. JZ 12/27/2013        
