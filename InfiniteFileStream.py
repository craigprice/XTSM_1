...
class Filestream(xstatus_ready.xstatus_ready, XTSM_Server_Objects.XTSM_Server_Object):
    ...
    def __init__(self, params={}):
        ...
        try: 
            self.zip_file = zipfile.ZipFile(self.location,
                                            mode='a',
    ...
    def output_log(self):
        ...
        outputs a log of recently written files
        """
        logstream = io.open(self.location_root + 'filestream_log.txt', 'a')
        time_format = '%Y-%m-%d %H:%M:%S'
        time1 = datetime.datetime.fromtimestamp(self.init_time).strftime(time_format)
        time2 = datetime.datetime.fromtimestamp(time.time()).strftime(time_format)
        timeheader= time1 + " through "+ time2
        msg = "\nThis is a log of file writes: This module has written the"
        msg = msg + "files below from the time period\n"
        msg = msg + timeheader + '\n\n'.join(self.filehistory)
        logstream.write(unicode(msg))
        logstream.close()
        
    def write_file(self, msg, comments='', prefix='', extension='.dat'):
        """
        writes a file to the zip archive.
        """
        fileName = str(prefix) + str(uuid.uuid1()) + str(extension)
        info = zipfile.ZipInfo(fileName, date_time=time.localtime(time.time()))
