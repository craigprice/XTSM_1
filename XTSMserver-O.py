...
TODO:
    permit standard command library calls with POST payloads on websocket connections (for faster exchanges on standard calls) ? is this done and working ?
...
import msgpack, msgpack_numpy, platform
msgpack_numpy.patch()
import XTSM_Server_Objects, XTSM_Transforms, live_content, xstatus_ready
import file_locations, server_initializations
    ...
    class XTSM_stack(collections.OrderedDict):
        ...
        def update(self,*args,**kwds):
            ...
            while len(self)>NUM_RETAINED_XTSM:
                self.heave()
                #self.popitem(last=False)      # JZ 8/14/2014
...
class MulticastProtocol(DatagramProtocol):
    ...
    def startProtocol(self):
        ...
        """
        #pdb.set_trace()
        self.transport.joinGroup("228.0.0.5", interface="10.1.1.124")
    ...
    def datagramReceived(self, datagram, address):
        ...
        if "server_id" not in datagram: print datagram
        try: 
...
class ClientManager(XTSM_Server_Objects.XTSM_Server_Object):
    ...
    def identify_client(self,protocol):
        ...
        non-resident (ephemoral) protocols are identified by ip address and header info
        """
        if protocol.resident: return protocol.peer
        pdb.set_trace()
    ...
    def connectLog(self,protocol):
        ...
        try: self.clients[pid].connectLog()
        except KeyError: self.clients.update({pid:self.client(params={"protocol":protocol})})

    def elaborateLog(self,protocol,request):
        return # temporary disable
        pid=self.identify_client(protocol)
        try: self.clients[pid].elaborateLog()
        except KeyError: self.clients.update({pid:self.client(params={"protocol":protocol})})
    ...
    def update_client_roles(self,request,role):
        ...
        else: self.client_roles[request['protocol'].peer].update({role:time.time()})
    role_timeouts={}
...
#                except KeyError: pass
    ...
    class client(XTSM_Server_Objects.XTSM_Server_Object):
        """
    ...
    class peer_server(client):
        """
        class to hold data regarding other XTSM servers on network
        """
        def __init__(self):
            ClientManager.client.__init__(self)
...
class CommandLibrary():
    ...
    def compile_active_xtsm(self, params):
        ...
        self.owner.clientManager.update_client_roles(params['request'],'active_XTSM_compiler')
...
class GlabPythonManager():
    ...
    def __init__(self):
        ...
        def hello(): print 'Listening on ports '+str(port)+' (standard HTTP) and '+str(wsport)+' (websocket)'
        ...
        try: 
            ...
            if init_code: exec(init_code) in globals(), locals()
        except KeyError: print "WARNING:: no supplementary server_initialization data present for this machine"
    ...
    def server_pong(self,payload):
        ...
        """
        self.clientManager.peer_servers.pinged(simplejson.loads(payload))
    ...
    def send(self,address,data):
        """
        sends data to a specified address, provided as a sting in the form ip:port, 
        a server_id string in the form of a uuid1 provided through peer discovery,
        or by role, in the form of a string "role:XXX" where XXX is one of:
            
        the send method will be chosen from among existing websocket connection
        if it exists, or establishes one if it does not.  Returns False if failed
        returns data if there was a response
        """
    ...
    def resolve_address(self,address):
        ...
        if (len(address.split(":"))==2) and (len(address.split("."))==3):
            return address
        if "role:" in address:
            try: return self.clients_by_role[address.split("role:")[1]]
